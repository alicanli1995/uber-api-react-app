...
import { uberAPI } from '../misc/UberAPI'
...
class UserSettings extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      const response = await uberAPI.getUserExtrasMe(keycloak.token)
  ...
  handleSave = async () => {
    ...
    try {
      ...
      await uberAPI.saveUserExtrasMe(keycloak.token, userExtra)
  ...
  render() {
    ...
    return (
      <Container>
        <Grid centered>
          <Grid.Row>
            <Segment style={{ width: '330px', marginTop : '100px' }}>
              <Form>
